    ...
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-text-field
          ...
          clearable
          dense
          @keyup="getTeamSectionsBySearchValue"
        ></v-text-field>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.mode ? "Add Section" : "Edit Section";
    },
    addOrEditHandler() {
  ...
  created() {
    this.reloadSectionsData("getAllTeamSections");
  },

  methods: {
    async reloadSectionsData(dispatch, payload) {
      let snapshot = (await this.$store.dispatch(dispatch, payload)).data;
      let tempArr = [];
      snapshot.map((section) => {
        let obj = {
          id: section._id,
          name_eng: section.name_eng,
    ...
    add() {
      this.$store.dispatch("addTeamSection", this.currentItem).then(() => {
        this.reloadSectionsData("getAllTeamSections");
        this.close();
        this.$notify({
    ...
    editItem() {
      ...
      this.$store.dispatch("editTeamSection", this.currentItem).then(() => {
        this.reloadSectionsData("getAllTeamSections");
        this.close();
        this.$notify({
          group: "mainActionsNotifications",
    ...
    async getTeamSectionsBySearchValue() {
      this.reloadSectionsData("getTeamSectionByName", {
        name: this.search,
      });
    },
  },
